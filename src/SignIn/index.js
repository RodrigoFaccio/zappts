...
export default function Login({ boxData }) {
  const schema = yup.object().shape({
    email:yup.string().email('o e-mail está incorreto').required('Preencha o campo email'),
    password:yup.string().min(6,'A senha não pode ter menos de 6 caracteres').required()
        ...
        <DivForm>
          <Form  onSubmit={handleSubmit(newUser)} noValidate>
            ...
            <GoogleLogin
              ...
              <Cadastro>
                New Invision?
                <Link style={{color:'#BAD1C7'}} to="/signup">Create Accout</Link>
              </Cadastro>
